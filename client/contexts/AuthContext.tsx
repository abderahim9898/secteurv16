...
export const AuthProvider = ({ children }: AuthProviderProps) => {
  ...
  const fetchUserData = async (firebaseUser: FirebaseUser) => {
    try {
      ...
      if (userDoc.exists()) {
        ...
        // Debug logging for farm assignment
        if (!userWithUid.fermeId) {
          console.warn('⚠️ User loaded without farm assignment:', {
            uid: userWithUid.uid,
          ...
          console.log('✅ User loaded with farm assignment:', {
            uid: userWithUid.uid,
            email: userWithUid.email,
            fermeId: userWithUid.fermeId,
            role: userWithUid.role
          });
        }
  ...
  const value: AuthContextType = {
    ...
    hasAllFarmsAccess: user?.role === 'superadmin' || user?.fermeId === 'all'
  };
